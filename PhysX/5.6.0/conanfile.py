...
class PhysXConan(ConanFile):
    ...
    def source(self):
        tools.get(
            "https://github.com/NVIDIA-Omniverse/PhysX/archive/refs/tags/107.0-physx-5.6.0.zip",
            sha256="3b8022df1fac232520582b2629a3452b40b0b1c6a2ff8573a4aba0706c699187",
        )
        strip_root(self)
    ...
    def configure_cmake(self):
        ...
        cmake.definitions["PX_ROOT_LIB_DIR"] = "lib"
        cmake.definitions["PX_ROOT_BIN_DIR"] = "bin"
        cmake.definitions["PX_OUTPUT_LIB_DIR"] = self.package_folder.replace("\\", "/")
        cmake.definitions["PX_OUTPUT_BIN_DIR"] = self.package_folder.replace("\\", "/")
    ...
    def package_info(self):
        lib_postfix = "" if self.options.shared else "_static"
        # PhysXFoundation
        self.cpp_info.components["physxfoundation"].libs = ["PhysXFoundation"+lib_postfix]
        if self.settings.os == "Linux":
        ...
        # PhysXCommon
        self.cpp_info.components["physxcommon"].libs = ["PhysXCommon"+lib_postfix]
        if self.settings.os == "Linux":
            self.cpp_info.components["physxcommon"].system_libs = ["m"]
        self.cpp_info.components["physxcommon"].requires = ["physxfoundation"]

        # PhysXPvdSDK
        self.cpp_info.components["physxpvdsdk"].libs = ["PhysXPvdSDK"+lib_postfix]
        self.cpp_info.components["physxpvdsdk"].requires = ["physxfoundation"]

        # PhysX
        self.cpp_info.components["physxmain"].libs = ["PhysX"+lib_postfix]
        if self.settings.os == "Linux":
        ...
        self.cpp_info.components["physxcharacterkinematic"].libs = ["PhysXCharacterKinematic"+lib_postfix]
        self.cpp_info.components["physxcharacterkinematic"].requires = ["physxfoundation", "physxcommon", "physxextensions"]

        # PhysXCooking
        self.cpp_info.components["physxcooking"].libs = ["PhysXCooking"+lib_postfix]
        if self.settings.os == "Linux":
        ...
        # PhysXVehicle
        self.cpp_info.components["physxvehicle"].libs = ["PhysXVehicle2"+lib_postfix]
        self.cpp_info.components["physxvehicle"].requires = ["physxfoundation", "physxpvdsdk", "physxextensions"]

        # PhysXExtensions
        self.cpp_info.components["physxextensions"].libs = ["PhysXExtensions"+lib_postfix]
