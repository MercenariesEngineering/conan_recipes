...
# Recipe based on "OpenImageIO/1.6.18@pierousseau/stable", redefined for openexr compatibility
class OpenimageioConan(ConanFile):
    name = "OpenImageIO"
    version = "1.6.18"
    license = "Modified BSD License"
    url = "http://www.openimageio.org"
    requires = "boost/1.64.0@conan/stable", "libjpeg-turbo/1.5.2@pierousseau/stable", "libpng/1.6.37@bincrafters/stable", "libtiff/4.0.9@bincrafters/stable", "OpenEXR/2.4.0@mercseng/stable", "zlib/1.2.11@conan/stable"
...
conan_basic_setup()
set(ILMBASE_HOME ${CONAN_OPENEXR_ROOT})
...
""" % (boost_libs, libjpeg))
    ...
    def build(self):
        ...
        #cmake.definitions["OPENEXR_CUSTOM_LIB_SUFFIX"] = "_d" if self.settings.build_type == "Debug" else ""
        if (self.settings.compiler == "gcc" and self.settings.compiler.version == 4.1):
