...
class PySide2(ConanFile):
    ...
    def _patch_sources(self):
        ...
        qt_include = self.dependencies["qt"].cpp_info.includedirs[0].replace("\\", "/")
        qt_library = self.dependencies["qt"].cpp_info.libdirs[0].replace("\\", "/")

        qt_components_found = """
set(Qt5Core_VERSION \""""+str(self.dependencies["qt"].ref.version)+"""\")
set(QT_INCLUDE_DIR \"""" + qt_include + """\")
set(QT_HEADERS_DIR \"""" + qt_include + """\")
set(QT_LIBRARY_DIR \"""" + qt_library + """\")
"""
...
"""
        ...
        for comp in self.dependencies["qt"].cpp_info.components:
            #libs = self.dependencies["qt"].cpp_info.components[comp].libs
            comp_name = "Qt5"+comp[2:]
            with open(os.path.join(self.build_folder, comp_name+"Config.cmake"), "w", encoding="utf-8") as outfile:
                outfile.write("""
set(""" + comp_name + """_FOUND TRUE)
set(""" + comp_name + """_VERSION_STRING \""""+qt_version+"""\")

set(""" + comp_name + """_INCLUDE_DIR \"""" + qt_include + """\" )
#get_target_property(""" + comp_name + """_INCLUDE_DIR Qt5::""" + comp[2:] + """ INTERFACE_INCLUDE_DIRECTORIES)
set(""" + comp_name + """_INCLUDE_DIRS \"${""" + comp_name + """_INCLUDE_DIR}")

#set(""" + comp_name + """_LIBRARIES """ + qt_library + """ )
set(""" + comp_name + """_LIBRARIES Qt5::""" + comp[2:] + """ )

get_target_property(""" + comp_name + """_DEFINITIONS Qt5::""" + comp[2:] + """ INTERFACE_COMPILE_DEFINITIONS)
""")
            with open(os.path.join(self.build_folder, comp_name+"ConfigVersion.cmake"), "w", encoding="utf-8") as outfile:
                outfile.write("""
...
if(PACKAGE_VERSION VERSION_LESS PACKAGE_FIND_VERSION)
    ...
    if(\""""+qt_version+"""\" MATCHES "^([0-9]+)\\.")
        ...
        set(CVF_VERSION_MAJOR \""""+qt_version+"""\")
    endif()
...
"""
        ...
        # Help cmake_helpers find Qt from Conan's CMakeDeps.
        replace_in_file(self, 
            os.path.join(self.source_folder, "sources", "cmake_helpers", "helpers.cmake"),
