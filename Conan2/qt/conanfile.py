...
#
# From: https://github.com/conan-io/conan-center-index/blob/2a3cb93885141024c1b405a01a79fb3abc239b12/recipes/qt/6.x.x/conanfile.py
# From: https://github.com/AcademySoftwareFoundation/aswf-docker/tree/main/packages/conan/recipes/qt/conanfile.py
...
class QtConan(ConanFile):
    ...
    def layout(self):
        ...
        # ASWF: DSOs in lib64
        self.cpp.build.libdirs = [self._lib]
        self.cpp.package.libdirs = [self._lib]
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_freetype", False) and not self.options.multiconfiguration:
            self.requires("freetype/2.13.2", transitive_libs=True) # ASWF: don't pick up system freetype
    ...
    def generate(self):
        ...
        tc.variables["INSTALL_LIBDIR"] = self._lib
    ...
    def _cmake_executables_file(self):
        ...
        return os.path.join(self._lib, "cmake", "Qt6Core", "conan_qt_executables_variables.cmake")

    @property
    def _cmake_entry_point_file(self):
        # ASWF: cmake modules in lib64
        return os.path.join(self._lib, "cmake", "Qt6Core", "conan_qt_entry_point.cmake")

    def _cmake_qt6_private_file(self, module):
        # ASWF: cmake modules in lib64
        return os.path.join(self._lib, "cmake", f"Qt6{module}", f"conan_qt_qt6_{module.lower()}private.cmake")
    ...
    def package(self):
        ...
        rmdir(self, os.path.join(self.package_folder, self._lib, "pkgconfig"))
        ...
        rm(self, "*.la*", os.path.join(self.package_folder, self._lib), recursive=True)
        ...
        # ASWF: cmake modules in lib64
        for m in os.listdir(os.path.join(self.package_folder, self._lib, "cmake")):
            if os.path.isfile(os.path.join(self.package_folder, self._lib, "cmake", m, f"{m}Macros.cmake")):
                continue
            if glob.glob(os.path.join(self.package_folder, self._lib, "cmake", m, "QtPublic*Helpers.cmake")):
                continue
            if m.endswith("Tools"):
                if os.path.isfile(os.path.join(self.package_folder, self._lib, "cmake", m, f"{m[:-5]}Macros.cmake")):
                    continue

            # ASWF: don't remove cmake modules
            # rmdir(self, os.path.join(self.package_folder, self._lib, "cmake", m))
        ...
        for target in targets:
            ...
            for path_ in [f"bin/{target}{extension}",
                          f"{self._lib}/{target}{extension}", # ASWF: DSOs in lib64
        ...
        if self.options.qtdeclarative:
            ...
            # ASWF: cmake modules in lib64
            save(self, os.path.join(self.package_folder, self._lib, "cmake", "Qt6Qml", "conan_qt_qt6_policies.cmake"), textwrap.dedent("""\
    ...
    def package_info(self):
        ...
        # ASWF: cmake modules in lib64
        for m in os.listdir(os.path.join(self._lib, "cmake")):
            ...
            if component_name in self.cpp_info.components:
                # ASWF: cmake modules in lib64
                module = os.path.join(self._lib, "cmake", m, f"{m}Macros.cmake")
                if os.path.isfile(module):
                    _add_build_module(component_name, module)

                # ASWF: cmake modules in lib64
                module = os.path.join(self._lib, "cmake", m, f"{m}ConfigExtras.cmake")
                if os.path.isfile(module):
                    _add_build_module(component_name, module)

                # ASWF: cmake modules in lib64 
                for helper_modules in glob.glob(os.path.join(self.package_folder, self._lib, "cmake", m, "QtPublic*Helpers.cmake")):
                    _add_build_module(component_name, helper_modules)
                self.cpp_info.components[component_name].builddirs.append(os.path.join(self._lib, "cmake", m))

            elif component_name.endswith("Tools") and component_name[:-5] in self.cpp_info.components:
                # ASFW: cmake modules in lib64
                module = os.path.join(self._lib, "cmake", f"{m}", f"{m[:-5]}Macros.cmake")
                if os.path.isfile(module):
                    _add_build_module(component_name[:-5], module)
                self.cpp_info.components[component_name[:-5]].builddirs.append(os.path.join(self._lib, "cmake", m))

        # ASWF: DSOs in lib64
        objects_dirs = glob.glob(os.path.join(self.package_folder, self._lib, "objects-*/"))
        ...
        if self.options.qtdeclarative:
            # ASWF: cmake modules in lib64
            build_modules_list.append(os.path.join(self.package_folder, self._lib, "cmake", "Qt6Qml", "conan_qt_qt6_policies.cmake"))
