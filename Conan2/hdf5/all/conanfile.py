...
class Hdf5Conan(ConanFile):
    ...
    def source(self):
        ...
        cmakelists_file = "%s/src/CMakeLists.txt" % self.source_folder
...
if (HDF5_ENABLE_THREADSAFE) 
    if (BUILD_STATIC_LIBS)
        ...
        if (MSVC)
            ...
            target_link_libraries (${HDF5_LIB_TARGET} PUBLIC PRIVATE INTERFACE pthread)
        endif ()
    endif()
    if (BUILD_SHARED_LIBS)
        ...
        if (MSVC)
            target_link_libraries (${HDF5_LIBSH_TARGET} PUBLIC PRIVATE INTERFACE Threads::Threads)
        else ()
            target_link_libraries (${HDF5_LIBSH_TARGET} PUBLIC PRIVATE INTERFACE pthread)
        endif ()
    endif()
endif ()
''')
    ...
    def generate(self):
        ...
        is_threadsafe = self.options.get_safe("threadsafe", False)
        self.output.info ("building "+("threadsafe" if is_threadsafe else "not threadsafe"))
